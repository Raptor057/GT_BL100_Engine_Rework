    ...
    public class GttSqlDB
    {
            ...
            int re_work, double bearing_Position, double arrow_Position, double hipot_IR, int cw_Speed, double amperage_CW, int ccw_Speed, double amperage_CCW, int ptc_Resistance)
        {
            await _con.ExecuteAsync("INSERT INTO [gtt].[dbo].[MotorsData] ([Modelo], [SerialNumber], [Volt], [RPM], [DateTimeMotor], [Rev], [LineCode], " +
                "[re_work], [Bearing_Position], [Arrow_Position], [Hipot_IR], [CW_Speed], [Amperage_CW], [CCW_Speed], [Amperage_CCW], [PTC_Resistance]) " +
                "VALUES(@model, @serialNumber, @Volt, @RPM, @DateTimeMotor, @Rev, @lineCode, @re_work, @bearing_Position, @arrow_Position, @hipot_IR, @cw_Speed, @amperage_CW, @ccw_Speed, @amperage_CCW, @ptc_Resistance)",
